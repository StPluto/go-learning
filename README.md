...
Функция `Range` итерирует над срезом или функцией `map` в конструкции `for`.
Когда происходит "ранжирование" над срезом, каждуй итераци. возвращается два значения это индекс и копия значения этого индекса.
...
Можно пропустить возвращение индекса или значения просто присвоив соответствующему значени `_`.
...
Если вам нуже только индекс то вы можете просто опустить второе значение.
...
Функция `map` связывает значения и ключи.
...
```
Нулевые значения `map` это `nil` для ключа, и `nil` для значения.
Функция `make` возвращает `map` заданного типа, инициализированного и готового для использования.

Литералы `map` похожи на литералы `struct`, но ключ обязателен.
...
```
Если тип верхнего уровня, это просто имя типа, то в элементах этого литерала его можно опустить.

Добавить элемент в `map m:` - `m[key] = elem`  
Вернуть элемент - `elem = m[key]`  
Удалить элемент - `delete(m, key)`  
Проверить есть ли элемент - `elem, ok = m[key]`  
Если `key` есть в `m`, то `ok` будет `true`, Иначе `ok` будет `false`.
Если `key` не в `map`, `elem` примет нулевое значение `map`.

#### Functions values 
Функции также могут быть переменными. Их можно передавать также как и переменные.
Также их можно использовать как аргументы других функций и возвращаемые значения.
```
...
Также в Golang есть функции замыкания (Closures).

Замыканиями называются функции которые обращаются к переменным вне своей области видимости. Они могут считывать значение переменной, а также присвоить им новое значение.
В этом смысле функция "привязана" к этим переменным.

```
func adder() func(int) int {
	sum := 0
	return func(x int) int { // Замыкание
		sum += x
		return sum
	}
}
```
В примере выше замыканием является возвращаемая функцией `adder()` функция.